...
py_compile.compile("YOUR CODE'S PATH HERE. example: C:\\Users\user1\\Desktop\python.py\\tweet_counter_basic5.py")
...
class TweetCounterGUI:
    ...
    def tweet(self):
        ...
        if tweet:
            self.tweet_counter.tweet_count += 1
            self.tweet_counter.add_tweet(tweet)
            message = f"Your tweet count is now {self.tweet_counter.tweet_count}."
    ...
    def update_tweet_history(self):
        ...
        for idx, tweet in enumerate(self.tweet_counter.tweets, start=1):
